...
bedtools coverage -a windows.bed -b ../Filtering/GATK_all_SNP_final_new_cand20mb.recode.vcf -counts > New/GATK_all_SNP_filtered_pass_cand_plusminus20mb_coverage_10kb
bedtools coverage -sorted -a windows.bed -b ../Filtering/GATK_all_SNP_final_new_prefiltered_cand20mb.vcf.recode.vcf -counts > New/GATK_all_SNP_filtered_cand_plusminus20mb_coverage_10kb

#obtaining the raw read depth (DP; pre-filtering) per sample per SNP:
grep -v '#' ../Filtering/GATK_all_SNP_final_new_prefiltered_cand.vcf.recode.vcf | cut -f 10-159 | awk '{for(i=1;i<=NF;++i) cols[i]} {for(i=1;i<=NF;i++) if(i in cols) {split($i,t,":"); $i=t[3]}}1' > tmp1
grep -v '##' ../Filtering/GATK_all_SNP_final_new_prefiltered_cand.vcf.recode.vcf | head -1 | cut -f 10-159 > IDs
cat IDs tmp1 > tmp2
grep -v '##' ../Filtering/GATK_all_SNP_final_new_prefiltered_cand.vcf.recode.vcf | cut -f 2 > pos
paste pos tmp2 > New/Read_Depth_Per_Ind
sed -i 's/ /\t/g' New/Read_Depth_Per_Ind
rm tmp* IDs pos
...
plink --vcf ../Filtering/GATK_all_SNP_final_new.vcf -aec --set-missing-var-ids @:# --chr CM014743.1 --thin 0.1 -r2 gz --ld-window 100 --ld-window-kb 50 --ld-window-r2 0 --out LD_decay_chr1 --mind 0.5 --geno 0.5
...
plink --vcf ../Filtering/GATK_all_SNP_final_new_cand21_22.recode.vcf --allow-extra-chr --set-missing-var-ids @:# --r2 --ld-window 100000 --ld-window-kb 1000 --ld-window-r2 0 --out 1mb_pairwise_ld
