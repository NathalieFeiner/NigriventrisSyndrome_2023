...
sampletable_BrownGreen <- sampletable[- grep("Black", sampletable$pigmentation),] #specific comparison
...
count_matrix_BrownGreen <- count_matrix[,- grep("black", colnames(count_matrix))] #specific comparison

dds_BrownGreen <- DESeqDataSetFromMatrix(countData = count_matrix_BrownGreen, colData = sampletable_BrownGreen, 
                                                                                     design = ~ pigmentation)
dds_BrownGreen <- DESeq(dds_BrownGreen)

#This code creates the plots in Extended Data Fig. 7d
d <- plotCounts(dds_BrownGreen, gene="MKX", intgroup="pigmentation",returnData=TRUE)
P1 <- ggplot(d, aes(x=pigmentation, y=count, color=pigmentation)) + 
  geom_boxplot() + geom_point(position=position_jitter(), size=3) + 
  scale_color_manual(values = c("#BD912D", "#72A042")) +
  theme_bw() + ggtitle("MKX") + theme(legend.position="none", axis.text.x=element_blank(), axis.title=element_blank())
d <- plotCounts(dds_BrownGreen, gene="RAB18", intgroup="pigmentation",returnData=TRUE)
...
d <- plotCounts(dds_BrownGreen, gene="ptchd32", intgroup="pigmentation",returnData=TRUE)
...
d <- plotCounts(dds_BrownGreen, gene="ptchd35", intgroup="pigmentation",returnData=TRUE)
...
d <- plotCounts(dds_BrownGreen, gene="pks1", intgroup="pigmentation",returnData=TRUE)
...
d <- plotCounts(dds_BrownGreen, gene="pks2", intgroup="pigmentation",returnData=TRUE)
...
d <- plotCounts(dds_BrownGreen, gene="pks3", intgroup="pigmentation",returnData=TRUE)
...
d <- plotCounts(dds_BrownGreen, gene="ptchd3X1", intgroup="pigmentation",returnData=TRUE)
...
d <- plotCounts(dds_BrownGreen, gene="ptchd34", intgroup="pigmentation",returnData=TRUE)
...
d <- plotCounts(dds_BrownGreen, gene="pks4", intgroup="pigmentation",returnData=TRUE)
...
d <- plotCounts(dds_BrownGreen, gene="ptchd3", intgroup="pigmentation",returnData=TRUE)
...
d <- plotCounts(dds_BrownGreen, gene="cyp2k6", intgroup="pigmentation",returnData=TRUE)
...
d <- plotCounts(dds_BrownGreen, gene="ymel1l", intgroup="pigmentation",returnData=TRUE)
...
d <- plotCounts(dds_BrownGreen, gene="pkssense", intgroup="pigmentation",returnData=TRUE)
...
d <- plotCounts(dds_BrownGreen, gene="pks5", intgroup="pigmentation",returnData=TRUE)
...
d <- plotCounts(dds_BrownGreen, gene="ptchd33", intgroup="pigmentation",returnData=TRUE)
...
d <- plotCounts(dds_BrownGreen, gene="mastl", intgroup="pigmentation",returnData=TRUE)
...
d <- plotCounts(dds_BrownGreen, gene="acbd5", intgroup="pigmentation",returnData=TRUE)
...
d <- plotCounts(dds_BrownGreen, gene="cetn2", intgroup="pigmentation",returnData=TRUE)
...
d <- plotCounts(dds_BrownGreen, gene="abil", intgroup="pigmentation",returnData=TRUE)
...
pdf("DEseq_CandGenes_BrownGreen.pdf", width = 20  , height = 8, useDingbats=FALSE)
grid.arrange(P1,P2, P3, P4,P5,P6,P7,P8,P9,P10,P11,P12,P13,P14,P15,P16,P17,P18,P19,P20,
             ncol=5)
dev.off()

#get differentially expressed genes
res_BrownGreen <- results(dds_BrownGreen, contrast=c("pigmentation","Brown","Green"), alpha=0.05)
resFilt_BrownGreen <- as.data.frame(res_BrownGreen)
resFilt_BrownGreen$Genes <- rownames(resFilt_BrownGreen)
...
keyvals.colour <- ifelse(rownames(res_BrownGreen) %in% candidates, "royalblue",
                         ifelse(rownames(res_BrownGreen) %in% color_genes, "black", "grey"))
#keyvals.colour <- c(rep('grey', 24927),rep('royalblue', 20))
names(keyvals.colour) <- ifelse(rownames(res_BrownGreen) %in% candidates, 'candidate_region', 
                                ifelse(rownames(res_BrownGreen) %in% color_genes,"color genes", 'genome-wide'))
keyvals.size <- ifelse(rownames(res_BrownGreen) %in% candidates, 3,
                       ifelse(rownames(res_BrownGreen) %in% color_genes,3, 2))
keyvals.alpha <- ifelse(rownames(res_BrownGreen) %in% candidates, 1,
                        ifelse(rownames(res_BrownGreen) %in% color_genes,1, 1))

P1 <- EnhancedVolcano(res_BrownGreen, lab = rownames(res_BrownGreen), x = 'log2FoldChange',y = 'padj',pCutoff = 0.05,
                selectLab = c('SPR','KIT','FZD2','EDNRB','DUSP2','DUSP14','DGAT2'), 
                xlim=c(-8,10), ylim=c(0,39), pointSize=keyvals.size, drawConnectors = TRUE, gridlines.minor=F,gridlines.major=F,
                widthConnectors = 0.75, lengthConnectors = unit(0.01, "npc"), shape=19,
                colAlpha=keyvals.alpha,labSize = 6.0, colCustom = keyvals.colour)

pdf("VolcanoPlot_BrownGreen.pdf", width = 8 , height = 8, useDingbats=FALSE)
grid.arrange(P1,
             ncol=1)
dev.off()
...
All_data <- merge(resFilt_BrownGreen, Gene_positions, by.x="Genes", by.y="Gene", all.x=T)
...
write.table(All_data_sig[,c(11,9,10)], "Outliers_RNAseq_browngreen", row.names = F, col.names = F, quote = F, sep="\t")
...
#All_data[All_data==0] <- 0.000000001
pdf("DEseq_manhattan_BrownGreen.pdf", width = 14  , height = 8, useDingbats=FALSE)
...
dev.off()
pdf("DEseq_manhattan_BrownGreen_Chr12.pdf", width = 14  , height = 8, useDingbats=FALSE)
...
pdf("DEseq_manhattan_BrownGreen_cand_RepColl.pdf", width = 14  , height = 8, useDingbats=FALSE)
grid.arrange(p1, ncol=1)
dev.off()

write.csv(All_data, file="resFilt_BrownGreen_pos.csv")
